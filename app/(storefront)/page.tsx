  ...
  return (
    <main className="min-h-screen bg-white">
            ...
            {categories.map((category: any) => {
              ...
              return (
                <Link key={category.slug} href={`/categories/${category.slug}`} className="group flex min-w-28 flex-col items-center text-center">
                  <span className="flex size-20 items-center justify-center rounded-full bg-[#FFF4F9] text-[#FF4F9A] shadow-sm transition duration-300 group-hover:-translate-y-1 group-hover:shadow-lg group-hover:shadow-[#FF4F9A]/18">
      ...
      <section className="mx-auto max-w-7xl px-4 py-9 sm:px-6 lg:px-8">
        <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/products?sort=featured" className="text-sm font-semibold text-[#FF4F9A]">Lihat Semua</Link>
          </div>
        </div>
        {flashSale.length ? (
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
              {brands.map((brand: any) => (
                <Link key={brand.slug} href={`/brands/${brand.slug}`} className="group rounded-2xl bg-white px-4 py-5 text-center shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-[#222222]/8">
                  <span className="relative mx-auto mb-3 flex size-16 items-center justify-center overflow-hidden rounded-full bg-[#FFF4F9] transition group-hover:scale-105">
            ...
            <div className="grid gap-4 md:grid-cols-4">
              {articles.map((article: any) => (
                <article key={article.slug} className="overflow-hidden rounded-2xl bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-[#222222]/8">
                  <div className="relative aspect-[4/3] bg-[#FFF4F9]">
      ...
      <footer className="border-t border-[#222222]/8 bg-white px-4 pb-24 pt-10 sm:px-6 md:pb-10 lg:px-8">
        <div className="mx-auto mb-10 grid max-w-7xl gap-4 rounded-3xl bg-[#FFF4F9] p-6 md:grid-cols-[1fr_420px] md:p-8">
          <div>
            <p className="text-sm font-bold uppercase tracking-wide text-[#FF4F9A]">Newsletter</p>
            <h2 className="mt-2 text-2xl font-semibold text-[#222222]">Dapatkan update promo grosir dan produk baru.</h2>
          </div>
          <form className="flex flex-col gap-3 sm:flex-row">
            <input aria-label="Email newsletter" placeholder="Email toko kamu" className="h-12 flex-1 rounded-full border border-[#222222]/10 bg-white px-5 text-sm outline-none focus:border-[#FF4F9A]" />
            <button className="h-12 rounded-full bg-[#FF4F9A] px-6 text-sm font-bold text-white">Daftar</button>
          </form>
        </div>
