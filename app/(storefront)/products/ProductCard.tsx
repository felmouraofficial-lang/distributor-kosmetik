...
export function ProductCard({ product }: { product: ProductCardProduct }) {
  ...
  return (
    <article className="group rounded-lg border border-[#222222]/8 bg-white p-2.5 transition hover:border-[#FF4F9A]/30 hover:shadow-lg hover:shadow-[#222222]/8">
      <Link href={`/products/${product.slug}`} className="block">
        <div className="relative aspect-square overflow-hidden rounded-md bg-[#FAFAFA]">
          {discount ? (
            <span className="absolute right-2 top-2 z-10 rounded-full bg-[#FF4F9A] px-2 py-1 text-[11px] font-bold text-white">
              -{discount}%
            </span>
          ) : null}
        ...
        <p className="mt-3 truncate text-xs font-semibold uppercase text-[#FF4F9A]">{product.brand.name}</p>
      ...
      <div className="mt-3 grid grid-cols-[1fr_38px] gap-2">
        ...
        <button className="flex h-9 items-center justify-center rounded-full border border-[#222222]/10 text-[#222222]/70 hover:border-[#FF4F9A]/35 hover:text-[#FF4F9A]" aria-label={`Tambah ${product.name} ke wishlist`}>
